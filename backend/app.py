...
def call_claude(sender: str, subject: str, body: str) -> dict:
    """Call the Anthropic API and parse the structured JSON response."""
    if client is None:
        raise RuntimeError(
    ...
    response = client.messages.create(
        ...
        messages=[{"role": "user", "content": build_user_prompt(sender, subject, body)}],
    )
...
def analyze_email(sender: str, subject: str, body: str) -> dict:
    ai_result = call_claude(sender, subject, body)
